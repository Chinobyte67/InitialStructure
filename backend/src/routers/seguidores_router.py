...
from src.dtos.seguidores_dto import CreateSeguidoresDTO, SeguidoresResponseDTO
...
def get_seguidor(seguidor_id: int, db: Session = Depends(get_db)):
    return SeguidoresController(db).get_seguidor_by_id(seguidor_id)

@router.get("/", response_model=list[SeguidoresResponseDTO])
def list_seguidores(db: Session = Depends(get_db)):
    return SeguidoresController(db).list_all_seguidores()
...
def create_seguidor(seguidor: CreateSeguidoresSchema, db: Session = Depends(get_db)):
    dto = CreateSeguidoresDTO(**seguidor.model_dump())
    return SeguidoresController(db).create_seguidor(dto)
