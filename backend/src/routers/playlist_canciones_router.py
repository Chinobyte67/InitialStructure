...
def create_playlist_canciones(payload: CreatePlaylistCancionesSchema, db: Session = Depends(get_db)):
    ...
    return PlaylistCancionesService(db).create_playlist_canciones(dto)
...
def delete_playlist_canciones(playlist_canciones_id: int, db: Session = Depends(get_db)):
    return PlaylistCancionesService(db).delete_playlist_canciones(playlist_canciones_id)

@router.put("/{playlist_canciones_id}", response_model=PlaylistCancionesResponseDTO)
def update_playlist_canciones(playlist_canciones_id: int, payload: UpdatePlaylistCancionesSchema, db: Session = Depends(get_db)):
...
def partial_update_playlist_canciones(playlist_canciones_id: int, payload: UpdatePlaylistCancionesSchema, db: Session = Depends(get_db)):
    dto = UpdatePlaylistCancionesDTO(**payload.model_dump())
    return PlaylistCancionesService(db).update_playlist_canciones(playlist_canciones_id, dto)
